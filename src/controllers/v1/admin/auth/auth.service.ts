...
import { ACCESS_TOKEN, accessTokenExpire, cookiesConfig, REFRESH_TOKEN, refresTokenExpire } from "src/constants/cookies.constants";
...
export class AuthService {
  ...
  async verifyAdminAndLogin({ email, password }: IAdminLoginDTO) {
    ...
    if (!isPasswordMatch) {
      throw new CustomError(HttpStatus.BAD_REQUEST, Messages.invalidCredentials)
    }
  ...
  async setCookiesToken(res: FastifyReply, tokens: Tokens) {
  ...
  async requestResetPassword(email: string) {
    ...
    if (!user) {
      throw new CustomError(HttpStatus.NOT_FOUND, Messages.recordNotFound('User'))
    }
    ...
    if (isTokenAlreadyCreated) {
      if (
        ...
        throw new CustomError(HttpStatus.BAD_REQUEST, Messages.tooManyPasswordResetRequest)
      }
      token = isTokenAlreadyCreated.token
      isTokenAlreadyCreated.count++
      await isTokenAlreadyCreated.save()
    } else {
      const tokenExpireTime = moment().add(this.forgetPasswordMinutes, "m").toDate()
      token = encryptData({
        userId: user.id
